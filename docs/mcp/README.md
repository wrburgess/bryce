...
nineteen tools over the same service layer and Zod schemas the [REST API](../api/README.md) and
...
It runs `initialize` → `tools/list` (asserts all nineteen tools) → `status` → a read-only
