...
export function buildMcpServer(deps: ServiceDeps): McpServer {
  ...
  server.registerTool(
    ...
    {
      description:
        "Run the digest job now for a window. window (default '1d') is one of the DATE windows 1d, 7d, 14d, 21d, 28d, 35d, 60d, ytd or the per-player GAME-COUNT windows last10games, last30games (each player over his own last N regular-season games); an unsupported value is rejected and nothing is sent. The report writes NO stat-line state, so re-running a window is always safe and sends the same content. A tag-free daily '1d' window is the SCHEDULED artifact FOR ITS LANE: it claims that lane's own once-per-date slot (so it never double-sends for a covered date, and a failed prior day is recovered on the next run), it goes to the lane's configured digest_to when it has one (otherwise the host DIGEST_TO), and it covers that lane's active members. list names the lane; omitting it means THE DEFAULT LANE, not every active player. So a second 1d send to one lane on one date returns already-sent-today — use force for a deliberate re-send — while two DIFFERENT lanes may each send on the same date. During Offseason Sleep the UNSCOPED 1d run becomes the weekly host heartbeat; a run that named a lane skips TODAY's digest with reason offseason-sleep instead, because the heartbeat is one signal per host, not one per lane. Sleep does NOT suspend recovery: a named lane that still owes an earlier day claims and mails that day first, then reports the skip for today. A lane deleted before the claim is refused (skipped, reason lane-deleted) and nothing is mailed, force included. Any OTHER window (a date window like 7d/ytd, or a game-count window like last10games) is an on-demand report: it takes no slot and records no delivery row, it goes to the host recipients, and it answers even during Offseason Sleep — an explicit report is a question, not the daily liveness signal. force (default false) applies only to the daily slot: it is a TEST send overriding the already-sent-today guard and the heartbeat's weekly rule. Overriding one of those makes the send a write-free replay that records nothing; forcing with no slot yet today, or over a failed slot, sends and records a delivery row normally. It does NOT override an in-flight claim held by another run — that still returns claimed-by-another-run. tags scopes the send to a cohort (e.g. 'level:dsl'); with list, the two intersect. A tag-scoped send is on-demand by definition — it takes no daily slot and records no delivery row, whatever the window — because the slot key has no tag dimension.",
