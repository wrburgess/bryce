...
// Pure, dependency-free modules only (no config, no service, no db), so preflight
// can validate a flag BEFORE a leaf is loaded and `startupDb` snapshots/migrates
...
export const COMMANDS: readonly Command[] = [
  ...
  leaf(["refresh"], "Refresh a lane's active players.", "sk refresh [--quiet|-q] [--list NAME|-l NAME]", "sk refresh --quiet", () => import("./refresh.js"), [flag("quiet", "Suppress live progress; print only the terminal summary.", ["q"]), listOption()]),
  leaf(["tick"], "Run whatever the lanes owe right now.", "sk tick [--quiet|-q]", "sk tick --quiet", () => import("./tick.js"), [flag("quiet", "Suppress per-stage lines; print only the terminal summary.", ["q"])]),
