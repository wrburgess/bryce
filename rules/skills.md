...
- **Never trim length by moving a load-bearing instruction behind a link** — Copilot does not follow external links, so an instruction relocated to a pointer is lost to it. Relocate only *reference* content (descriptions, history) to save space; keep instructions (the umbrella closing-keyword rule, the invocation distinction, the quality gate) resident in the file. **A rule's *scoping condition* — the clause naming when it applies, though not the example illustrating it — is instruction, not reference**, so it stays resident too: relocating the condition leaves a correct-sounding imperative a link-blind reader cannot tell applies to their situation, which is worse than the length it saved. *(Provenance: issue #151 / PR #156; extend per host.)*
