...
- **Never reuse a *coercing* validator at a typed-JSON boundary** — because a schema written for string inputs (a REST path/query param) silently coerces malformed types when reused for a JSON body or a typed tool call: `z.coerce.number()` turns `true`→`1` and a singleton array `[123]`→`123`, so a malformed field *passes* validation and the code acts on the wrong value instead of rejecting the call. Match the validator's coercion to the boundary's real wire type — non-coercing for typed JSON, coercing only where the input genuinely arrives as a string — even when it means a second, stricter schema alongside the string-boundary one. *(Provenance: #68 / PR #84; extend per host.)*
