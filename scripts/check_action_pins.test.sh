#!/usr/bin/env bash
#
# Tests for check_action_pins.rb (the supply-chain action-pin guard — issue #59).
#
# Run:  bash scripts/check_action_pins.test.sh
# Exit: 0 if every case passes, 1 otherwise.
#
# Drives the REAL Ruby script against FIXTURE workflow trees built in a temp dir,
...
SCRIPT="$SCRIPT_DIR/check_action_pins.rb"
...
#    Detect non-ASCII bytes with Ruby (portable; BSD grep lacks -P).
R="$TMP/ascii"; mk_workflow "$R" app.yml "actions/checkout@v4"
ascii_out="$(LANG=C LC_ALL=C ruby "$SCRIPT" --root "$R" 2>&1)"
if printf '%s' "$ascii_out" | ruby -e 'exit(STDIN.read.bytes.all? { |b| b < 128 } ? 0 : 1)'; then
